...
export default function PptToPdfTool() {
    ...
    const handleSubmit = async () => {
        if (!file) return alert("Upload a PPT file first!");
        ...
        try {
            ...
            a.href = url;
            a.download = (outputName || file.name.replace(/\.(pptx|ppt)$/i, "")) + ".pdf";
            ...
            console.error(e);
            alert("Failed to convert PPT to PDF.");
        } finally {
            setLoading(false);
        }
    };
            ...
            <div style={{ marginBottom: 16 }}>
                <input
                    ...
                    onChange={(e) => {
                        ...
                        setFile(f);
                        if (f) setOutputName(f.name.replace(/\.(pptx|ppt)$/i, ""));
                    }}
                />
            </div>
            <div style={{ marginBottom: 16 }}>
